...
class SwarmAnalyzer(object):
    def __init__(self, path2boptPython):
        self.path2boptPython = path2boptPython if path2boptPython[-1] == '/' else path2boptPython+'/'

    def get_folders_of_collections(self):
        folders_of_collections = []
        for el in os.listdir(self.path2boptPython):
    ...
    def call_selection_criteria(ad, upper_bound_objectives, best_idx=None, proj_name=None):
        return selection_criteria(ad, 
                                ...
                                ad.solver.swarm_data_container.project_names, 
                                upper_bound_objectives=upper_bound_objectives, 
                                    ...
                                    Q=ad.solver.spec_input_dict['Qs'], p=ad.solver.spec_input_dict['p'])
    ...
    def inspect_swarm_and_show_table_plus_Pareto_front(self, ad_list):
        ...
        for ind, ad in enumerate(ad_list_sorted):
            ...
            scatter_handle, more_info, auto_optimal_designs = main_utility.pareto_front_plot_script(ad.solver.swarm_data, fig, ax, marker, label, z_filter=16, bool_return_more_details=True) # z_filter=20 filtered individual that has OC larger than 20
    ...
    def select_optimal_designs_manually(self, st, session_state, global_recovered_values, 
        ...
        for ind, specification in enumerate(selected_specifications):
            ...
            recording_text_input = utility_streamlit.make_recording_widget(st.text_input, global_recovered_values)
            if text in session_state.default_value_global_recovered_values.keys():
                user_input_upper_bounds_4filter = recording_text_input(\
                    text,  # label
                    session_state.default_value_global_recovered_values[text] # st.empty() # global_recovered_values[text] # default value 
                )
                print('\t[default             ]', session_state.default_value_global_recovered_values[text])
            else:
                user_input_upper_bounds_4filter = recording_text_input(\
                    text,  # label
