...
import CrossSectInnerConsequentSinglePoleRotor
...
class bearingless_consequentsinglePole_template(inner_rotor_motor.template_machine_as_numbers):
    ...
    def __init__(self, fea_config_dict, spec_input_dict):
        # 初始化父类
        super(bearingless_consequentsinglePole_template, self).__init__(fea_config_dict, spec_input_dict)
...
class bearingless_consequentsinglePole_design_variant(inner_rotor_motor.variant_machine_as_objects):
    ...
    def __init__(self, template=None, x_denorm=None, counter=None, counter_loop=None):
        ...
        # 初始化父类
        super(bearingless_consequentsinglePole_design_variant, self).__init__(template, x_denorm, counter, counter_loop)
        ...
        # Parts
        self.rotorCore = CrossSectInnerConsequentSinglePoleRotor.CrossSectConsequentSinglePoleRotor(
                            name = 'ConsequentSinglePoleRotor',
                            ...
                            p = template.SI['number_of_rotor_pole_pairs'], # Set pole-pairs to 2
                            s = template.SI['no_segmented_magnets'], # Set magnet segments/pole to 4
                            location = Location2D.Location2D(anchor_xy=[0,0], deg_theta=0))

        self.shaft = CrossSectInnerConsequentSinglePoleRotor.CrossSectShaft(name = 'Shaft',
                                                      ConsequentSinglePole_rotor = self.rotorCore
                                                    )

        self.rotorMagnet = CrossSectInnerConsequentSinglePoleRotor.CrossSectConsequentSinglePoleMagnet( name = 'ConsequentSinglePole',
                                                      ConsequentSinglePole_rotor = self.rotorCore
                                                    )
        ...
        self.sleeve = CrossSectInnerConsequentSinglePoleRotor.CrossSectSleeve(
                            name = 'Sleeve',
                            ConsequentSinglePole_magnet = self.rotorMagnet,
        ...
        if True:
            ...
            print('[bearingless_consequentsinglePole_design.py] [CSPPM JMAG] InitialRotationAngle :',(deg_pole_span-GP['deg_alpha_rm'].value)*0.5, - deg_pole_span*0.5, + wily.deg_winding_U_phase_phase_axis_angle,  + deg_pole_span)
            print('[bearingless_consequentsinglePole_design.py] [CSPPM JMAG] InitialRotationAngle =',(deg_pole_span-GP['deg_alpha_rm'].value)*0.5  - deg_pole_span*0.5  + wily.deg_winding_U_phase_phase_axis_angle   + deg_pole_span, 'deg')
    ...
    def check_invalid_design(self, GP, SI):
        ...
        if GP['mm_d_rp'].value > GP['mm_d_pm'].value:
            ...
            msg = '[Warning from bearingless_consequentsinglePole_design.py]: Inter-pole notch depth mm_d_rp cannot be larger than mm_d_pm or else the sleeve cannot really hold or even touch the PM. So mm_d_rp is set to mm_d_pm.'
            print(msg)
