...
class bearingless_VconsequentPole_template(inner_rotor_motor.template_machine_as_numbers):
    ...
    def __init__(self, fea_config_dict, spec_input_dict):
        # 初始化父类
        super(bearingless_VconsequentPole_template, self).__init__(fea_config_dict, spec_input_dict)

        # 基本信息
        self.machine_type = 'VCPPM'
        self.name = '__VCPPM'
    ...
    def Bianchi2006(self, fea_config_dict, SI, GP, EX):
        ...
        stator_outer_diameter_Dse = 0.010 # this is related to the stator current density and should be determined by Js and power.
        sleeve_length = 1
...
class bearingless_VconsequentPole_design_variant(inner_rotor_motor.variant_machine_as_objects):
    ...
    def __init__(self, template=None, x_denorm=None, counter=None, counter_loop=None):
        ...
        # 初始化父类
        super(bearingless_VconsequentPole_design_variant, self).__init__(template, x_denorm, counter, counter_loop)
        ...
        self.rotorMagnet = CrossSectVShapeConsequentPoleRotor.CrossSectVShapeConsequentMagnet( name = 'VConsequentPole',
                                                      VConsequentPole_rotor = self.rotorCore
                                                    )
        ...
        if True:
            ...
            print('[bearingless_VShapeconsequentPole_design.py] [VCPPM JMAG] InitialRotationAngle :',(deg_pole_span-GP['deg_alpha_rm'].value)*0.5, - deg_pole_span*0.5, + wily.deg_winding_U_phase_phase_axis_angle,  + deg_pole_span)
            print('[bearingless_VShapeconsequentPole_design.py] [VCPPM JMAG] InitialRotationAngle =',(deg_pole_span-GP['deg_alpha_rm'].value)*0.5  - deg_pole_span*0.5  + wily.deg_winding_U_phase_phase_axis_angle   + deg_pole_span, 'deg')
