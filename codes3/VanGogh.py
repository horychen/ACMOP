...
class VanGogh_TikZPlotter():
    ...
    def prepareSection(self, list_regions, bMirrorMerge=True, bRotateMerge=True, color=None, **kwarg):
        ...
        for list_segments in list_regions:
            for segment in list_segments:
                ...
                路径.append(pyx.path.closepath()) # see https://stackoverflow.com/questions/50570100/filling-a-closed-curve-in-pyx-python-failing
            ...
            if self.iRotateCopy != 0:
                for i in range(self.iRotateCopy):
                    ...
                    if self.bMirror == True:
                        self.c.fill(路径, [color_setting, pyx.trafo.mirror(0), pyx.trafo.rotate(i*360/self.iRotateCopy)])
    ...
    def draw_line(self, p1, p2, ax=None, bool_stroke=False, **kwarg):
        ...
        if bool_stroke:
            ...
            self.c.fill(pyx.path.circle(p1[0], p1[1], 1*0.075)) # use this if THICK is used. 这里前面乘的7是和pyx.unit.set(wscale=7)一致的。按照官方manual，如果用的join，就不用担心连接处断裂，但是咱们默认都是用的add。
            self.c.fill(pyx.path.circle(p2[0], p2[1], 1*0.075)) # use this if THICK is used. 这里前面乘的7是和pyx.unit.set(wscale=7)一致的。按照官方manual，如果用的join，就不用担心连接处断裂，但是咱们默认都是用的add。
    ...
    def draw_arc(self, startxy, endxy, centerxy=(0,0), bool_stroke=False, **kwarg): # ccw=-1, 
        ...
        if True:
            ...
            if bool_stroke:
                self.c.stroke(路径, [pyx.color.rgb.black, pyx.style.linewidth.THICK]) # https://pyx-project.org/manual/connector.html?highlight=relangle
            ...
            if ccw == 1:
                ...
                路径 = pyx.path.path(pyx.path.arcn(centerxy[0], centerxy[1], radius, angle_start, angle_end-angle_start))
            if bool_stroke:
                pass
                # See PyX_Utility.py
                # Or
    ...
    def pyx_draw_path(tool_tikz, path, sign=1, bool_exclude_path=False, bool_stroke=False):
        ...
        if bool_exclude_path == False:
            if len(path) == 4: # line
                ...
                p = tool_tikz.draw_arc(path[:2], path[2:4], path[4:6], relangle=sign*path[6], untrack=True, ccw=sign, bool_stroke=bool_stroke)
            return p
...
if __name__ == '__main__':
    ...
    if bool_4pole_2pole: # 4 pole motor
        ...
        l_rightlayer1 = [ 'C', 'A', 'A', 'A', 'A', 'B', 'B', 'B', 'B', 'C', 'C', 'C', 'C', 'A', 'A', 'A', 'A', 'B', 'B', 'B', 'B', 'C', 'C', 'C'][::-1]
        l_rightlayer2 = [ '-', '+', '+', '+', '+', '-', '-', '-', '-', '+', '+', '+', '+', '-', '-', '-', '-', '+', '+', '+', '+', '-', '-', '-'][::-1]
        l_leftlayer1  = [ 'C', 'C', 'C', 'C', 'A', 'A', 'A', 'A', 'B', 'B', 'B', 'B', 'C', 'C', 'C', 'C', 'A', 'A', 'A', 'A', 'B', 'B', 'B', 'B'][::-1]
        l_leftlayer2  = [ '-', '-', '-', '-', '+', '+', '+', '+', '-', '-', '-', '-', '+', '+', '+', '+', '-', '-', '-', '-', '+', '+', '+', '+'][::-1]
