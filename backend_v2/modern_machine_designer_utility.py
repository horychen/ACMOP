...
class CairoDrawer(object):
    ...
    def prepareSection(self, region_dict, color=None, **kwargs):
        ...
        # print(f"DEBUG: prepareSection received list_regions with {len(list_regions)} elements. First element: {list_regions[0] if list_regions else 'None'}")
        ...
        for i in range(loop_count):
            ...
            self.ctx.set_line_width(1.0 / self.scale) # adjust for scale
            # print(f"DEBUG: Path extents before stroke: {self.ctx.path_extents()}")
            self.ctx.stroke()
