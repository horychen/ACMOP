...
def json_dump_ignoring_unserializable(obj, *args, **kwargs):
    """
    Dumps a Python object to a JSON string, ignoring objects that cannot be serialized.
    ...
    def default_serializer(o):
        ...
        Returns a string representation indicating the object type, or raises TypeError.
        """
        try:
            ...
            return f"<<unserializable: {type(o).__qualname__}>>"

    # Pass the custom default_serializer to json.dumps
    return json.dumps(obj, default=default_serializer, *args, **kwargs)
...
class Pyrhonen_design(object):
    def __init__(self, im, bounds=None):
        ...
        '''
        # rotor_slot_radius = (2*pi*(template.SI['GP']['mm_r_ro'].value - Length_HeadNeckRotorSlot)*1e-3 - rotor_tooth_width_b_dr*Qr) / (2*Qr+2*pi)
        # => rotor_tooth_width_b_dr = ( 2*pi*(template.SI['GP']['mm_r_ro'].value - Length_HeadNeckRotorSlot)*1e-3  - rotor_slot_radius * (2*Qr+2*pi) ) / Qr
        # Verified in Tran2TSS_PS_Opti.xlsx.
            # ( 2*pi*(im.template.SI['GP']['mm_r_ro'].value - im.Length_HeadNeckRotorSlot)  - im.Radius_of_RotorSlot * (2*Qr+2*pi) ) / Qr
        ...
        self.rotor_tooth_width_b_dr         = ( 2*pi*(im.template.SI['GP']['mm_r_ro'].value - im.Length_HeadNeckRotorSlot)  - im.Radius_of_RotorSlot * (2*Qr+2*pi) ) / Qr
...
def get_windage_loss(im_variant, mm_stack_length, TEMPERATURE_OF_AIR=75):
    ...
    if Shaft[2] == 0: # free running cylinder
        ...
        windage_loss_radial = c_W*math.pi*rho_Air* Omega**3 * R**5 * (1.+L/R)
    ...
    print('\t windage_loss_total = windage_loss_radial + windage_loss_axial = {:.2f} [W] = {:.2f} + {:.2f} [W]'.format(
        windage_loss_total, windage_loss_radial, windage_loss_axial))
...
class suspension_force_vector(object):
    ...
    def __init__(self, force_x, force_y, range_ss=None): # range_ss means range_steadystate
        ...
        self.force_ang = []
        temp_force_ang = np.arctan2(force_y, force_x) / math.pi * 180 # [deg]
        ...
        self.ss_avg_force_angle     = np.arctan2(self.ss_avg_force_vector[1], self.ss_avg_force_vector[0]) / math.pi * 180
        ...
        self.force_err_ang_new_way = angle_error_nick.angle_error(np.ones(len(self.force_ang))*self.ss_avg_force_angle, np.array(self.force_ang))
...
def collect_jmag_Tran2TSSProlong_results(im_variant, path_prefix, fea_config_dict, axeses, femm_solver_data=None):
    ...
    # Tran2TSS
    ################################################################
    number_of_total_steps = 1 + fea_config_dict['number_of_steps_1stTSS'] + fea_config_dict['designer.number_of_steps_2ndTSS']
    ...
    sfv = suspension_force_vector(ForConX_list, ForConY_list, range_ss=fea_config_dict['designer.number_of_steps_2ndTSS']) # samples in the tail that are in steady state
    str_results, torque_average, normalized_torque_ripple, ss_avg_force_magnitude, normalized_force_error_magnitude, force_error_angle = \
        add_plots( axeses, dm,
    ...
    if femm_solver_data is not None:
        ...
        time_list = rotor_position_in_deg/180.*math.pi / im_variant.template.SI['EX']['Omega']
...
def get_copper_loss_Bolognani(stator_slot_area, rotor_slot_area=None, STATOR_SLOT_FILL_FACTOR=0.5, ROTOR_SLOT_FILL_FACTOR=1.0, TEMPERATURE_OF_COIL=75, copper_loss_parameters=None):
    ...
    slot_pitch_pps = math.pi * (stator_inner_diameter_D + slot_height_h_t) / Q
    kov = 1.8 # \in [1.6, 2.0]
    # On 2021-03-20, Imthiaz Ahmed: there should be additional 0.5 in here:
    end_winding_length_Lew = math.pi*0.5* 0.5*(slot_pitch_pps + tooth_width_w_t) + slot_pitch_pps*kov * (coil_pitch_yq - 1)
        # end_winding_length_Lew = math.pi*0.5 * (slot_pitch_pps + tooth_width_w_t) + slot_pitch_pps*kov * (coil_pitch_yq - 1) # wrong
    ...
    if rotor_slot_area is not None:
        ...
        Q                        = self.im.Qr
        # the_radius_m             = 1e-3*(self.im.template.SI['GP']['mm_r_ro'].value - self.im.Radius_of_RotorSlot)
        ...
        rotor_outer_diameter_Dor = 2*(self.im.template.SI['GP']['mm_r_ro'].value*1e-3)
        slot_height_h_t = self.im.rotor_slot_height_h_sr
        slot_pitch_pps = math.pi * (rotor_outer_diameter_Dor - slot_height_h_t) / Q
        kov = 1.6 
        end_winding_length_Lew = math.pi*0.5 * (slot_pitch_pps + tooth_width_w_t) + slot_pitch_pps*kov * (coil_pitch_yq - 1)
                ...
                \\begin{{table*}}[!t]
                  ...
                  \\centering
                    \\begin{{tabular}}{{rl}}
                      ...
                      Rotor OD [mm]                                         & {2*self.sw.im.template.SI['GP']['mm_r_ro'].value         :.1f} \\\\
