...
const MotorWindingSimulator = () => {
    ...
    const wires: Record<string, { d_bare: number; d_od: number; color: string }> = {
    ...
    const toCartesian = (r: number, theta: number, S: number) => ({
    ...
    const windingData = useMemo(() => {
        ...
        const results: Array<{ x: number, y: number, r: number, color: string }> = [];
...
const StatBox = ({ label, value, unit, color = "text-white", sub }: StatBoxProps) => (
