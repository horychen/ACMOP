...
class CrossSectInnerRotorStator:
    ...
    def draw(self, drawer, bool_draw_whole_model=False):
        ...
        Radius_InnerStatorYoke = sqrt(P5[0]**2 + P5[1]**2)
...
class CrossSectInnerRotorClosedSlotStator:
    def __init__(self,
                    name  = 'StatorCore',
                    color = '#BAFD01',
                    mm_r_so      = 40, # outer radius of stator teeth: class type DimLinear
    ...
    def draw(self, drawer, bool_draw_whole_model=False):
        ...
        P1 = [r_si, 0]
        P1_Rotate = [r_si*cos(alpha_slot_span*0.5), r_si*-sin(alpha_slot_span*0.5)]
        P2 = [r_si+d_sts, 0]
        P2_Rotate = [r_si+d_sts*cos(alpha_slot_span*0.5), (r_si+d_sts)*-sin(alpha_slot_span*0.5)]
        P3 = [r_si+d_sts, w_st*0.5]
        P4 = [r_si+d_sts+d_st, w_st*0.5]

        P5 = [r_so, 0]
        P5_Rotate = [r_so*cos(alpha_slot_span*0.5), r_so*-sin(alpha_slot_span*0.5)]

        P6 = [r_si+d_sts+d_st, 0]
        P6_Rotate = [r_si+d_sts+d_st*cos(alpha_slot_span*0.5), (r_si+d_sts+d_st)*-sin(alpha_slot_span*0.5)]
        ...
        self.list_region = [list_segments]
        return {'innerCoord': self.innerCoord, 'list_regions':[list_segments], 'mirrorAxis': [P1, P5]}
...
class CrossSectInnerRotorClosedSlotStatorWinding(object):
    def __init__(self, 
                    name = 'Coils',
                    color = '#3D9970',
                    stator_core = None
...
class CrossSectInnerRotorStator_PMAtYoke:
    ...
    def draw(self, drawer, bool_draw_whole_model=False):
        ...
        r_sy = Radius_InnerStatorYoke = sqrt(P5[0]**2 + P5[1]**2)
...
class CrossSectStatorMagnetAtYoke:
    ...
    def draw(self, drawer, bool_draw_whole_model=False):
        ...
        P0 = [r_sy, 0]
        alpha_pm_depth = 2*np.arcsin(0.5*mm_d_pm / r_sy)
        mm_w_pm = d_sy - self.statorCore.mm_difference_pm_yoke
...
class CrossSectInnerRotorStator_PMAtToothBody:
    ...
    def draw(self, drawer, bool_draw_whole_model=False):
        ...
        r_so = r_sy + d_sy
        P_PM = [sqrt((r_so)**2 - P1[1]**2) - self.mm_difference_pm_yoke, P1[1]]
...
class CrossSectStatorMagnetAtToothBody:
    ...
    def draw(self, drawer, bool_draw_whole_model=False):
        ...
        r_so = r_sy + d_sy
        P_PM = [sqrt((r_so)**2 - P1[1]**2) - self.statorCore.mm_difference_pm_yoke, P1[1]]
        ...
        mm2_magnet_area = sqrt( (         P1[0]-P_PM[0])**2 + (         P1[1]-P_PM[1])**2 ) \
                             * sqrt( (P_PM_Mirror[0]-P_PM[0])**2 + (P_PM_Mirror[1]-P_PM[1])**2 ) * Q # Note Q = 2*pe
        self.mm2_magnet_area = mm2_magnet_area
