...
if __name__ == '__main__':
    ...
    pu.pyx_text([x-5+3,  ell_limit], r'$\ell=\psi^*$', settings=['RawSienna'])
    pu.pyx_text([x-8+3, -ell_limit], r'$-\ell=-\psi^*$', settings=['RawSienna'])
    ...
    def f1(t):
        return  psi_mu_max*math.sin(2*np.pi*(freq1/x) * t )
    list_of_saturated_time_before_t2 = []
    list_of_saturated_time_after_t2 = []
    def f2(t):
        val  =  psi_mu_min*math.sin(2*np.pi*(freq2/x) *(t-pt1[0]))
    ...
    graph.draw( pu, [(t, f2(t)) for t in np.arange(pt1[0], pt3[0]+.1, 0.25)] )
    ...
    tbox = pu.pyx_text( [pt1[0]/2+3, ell_limit+1.5], 
    ...
    pu.pyx_marker(pt2); pu.pyx_text(text_bias(pt2), r'$t_2$')
    pu.pyx_marker(pt3); pu.pyx_text(text_bias(pt3,xbias=1), r'$t_3$')
