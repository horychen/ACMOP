...
class bearingless_spmsm_template(inner_rotor_motor.template_machine_as_numbers):
    ...
    def __init__(self, fea_config_dict, spec_input_dict):
        ...
        childGP = OrderedDict({
            ...
            "mm_d_ri"           : acmop_parameter("fixed",     "rotor_iron (back iron) depth",  None, [None, None], lambda GP,SI:None),
    ...
    def Bianchi2006(self, fea_config_dict, SI, GP, EX):
        ...
        stator_yoke_height_h_ys = air_gap_flux_density_Bg * np.pi * stator_inner_diameter_Dis * alpha_pm * B_max * lef / (2*stator_yoke_flux_density_Bsy * 2*SI['p'] *0.95*(lef-2*delta_0))
        ...
        stator_tooth_width_b_ds = alpha_pm * B_max * np.pi * stator_inner_diameter_Dis*(lef-2*delta_0) / (stator_tooth_flux_density_Bst* SI['Qs']*0.95*lef)
        # print(stator_tooth_width_b_ds, alpha_pm , B_max , np.pi , stator_inner_diameter_Dis, lef, delta_0, stator_tooth_flux_density_Bst)
